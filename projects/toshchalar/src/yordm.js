...
const yutq          = document.querySelector(".yutq");
const ism           = document.querySelector(".ism");

// LOGICAL VARIABLES

newBtn.addEventListener("click", () => {
  name1.innerText = prompt("ism Kiriting1");
  name2.innerText = prompt("ism Kiriting2");
});
...
function handleHold(e) {
  ...
  if (totalScore >= WINNER_SCORE) {
    ...
    if (player === player1) {
      ...
      yutq.style.display = "flex";
      ism.innerText = name1.innerText;
      
    } else {
      men.style.display = "none";
      yutq.style.display = "flex";
      ism.innerText = name2.innerText;
    }
  } else {
    changePlayer(currentScore, player);
  }
}
